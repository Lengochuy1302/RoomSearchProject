...
public class RoomModel implements Parcelable {
    ...
    double latitude, longtitude, length, width;
    int rentalCosts;
    ...
    private void getPartListRoom(DataSnapshot dataSnapshot, IMainRoomModel mainRoomModelInterface, int quantityRoomToLoad, int quantityRoomLoaded) {
        ...
        for (String RoomID : listRoomsID) {
            ...
            for (DataSnapshot valueRoomPrice : dataSnapshotRoomPrice.getChildren()) {
                ...
                int price = valueRoomPrice.getValue(int.class);
            ...
            if (roomModel.getViews() >= 4 && roomModel.getListCommentRoom().size() >=4) {
                nodeRoot.child("Room").child(RoomID).child("authentication").setValue(true);
            } else {
                nodeRoot.child("Room").child(RoomID).child("authentication").setValue(false);
            }
    ...
    public void addRoom(String UID, RoomModel roomModel, List<String> listConvenient, List<String> listPathImage
            , int electricBill, int warterBill, int InternetBill, int parkingBill, ICallBackFromAddRoom iCallBackFromAddRoom, Context context) {
        ...
        for (String pathImage : listPathImage) {
            ...
            StorageReference storageReference = FirebaseStorage.getInstance().getReference()
                    .child("Images/"+ calendar.getTimeInMillis()+".png");
                    ...
                    storageReference.getDownloadUrl().addOnSuccessListener(new OnSuccessListener<Uri>() {
                        ...
                        public void onSuccess(Uri uri) {
                            ...
                            if (count[0] == listPathImage.size()) {
                                ...
                                addDetailRoomPrice(RoomID, electricBill, warterBill, InternetBill, parkingBill, (int) roomModel.getRentalCosts());
                                ...
                                //Tạo đường dẫn file
                                nodeRoot.child("RoomCompressionImages").child(RoomID).push().setValue(dowloadURL);
    ...
    private void addDetailRoomPrice(String roomID, int electricBill, int warterBill, int InternetBill, int parkingBill, int roomBill) {
        //Thêm tiền nước
    ...
    public void ListRoomUserroom(final IMainRoomModel mainRoomModelInterface, String userID, int quantityRoomToLoad, int quantityRoomLoaded) {
        Query nodeRoomUser = nodeRoot.child("Room")
                .orderByChild("owner")
                .equalTo(userID);
    ...
    public void addToFavoriteRooms(String roomId, IMainRoomModel iMainRoomModel, String UID) {
        ...
        nodeFavoriteRooms.child(currentUser.getUid()).child(roomId).child("time").setValue(date).addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
    ...
    public void removeFromFavoriteRooms(String roomId, IMainRoomModel iMainRoomModel, String UID) {
        ...
        nodeFavoriteRooms.child(currentUser.getUid()).child(roomId).removeValue().addOnCompleteListener(new OnCompleteListener<Void>() {
            @Override
            public void onComplete(@NonNull Task<Void> task) {
                if (task.isSuccessful()) {
    ...
    public void postRoomStep2Update(String roomId, String typeId, boolean genderRoom, long maxNumber, float width, float length,
                                    int priceRoom, int electricBill, int warterBill, int InternetBill, int parkingBill, IPostRoomUpdateModel iPostRoomUpdateModel) {
                                ...
                                if (header.equals("https://firebasestorage") == false) {
                                    ...
                                    if (new File(path).exists() == true) {
                                        ...
                                        try {
                                            ...
                                            //Up load hình
                                            UploadTask uploadTask = CompressStorage.putBytes(imagebyte);
